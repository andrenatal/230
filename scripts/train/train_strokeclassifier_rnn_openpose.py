...
import os
import json
from scripts.tools.customcallback import CustomCallback
...
log_dir = "logs/openpose/classification/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
tensorboard_callback = TensorBoard(log_dir=log_dir, histogram_freq=1)
early_stopping_callback = EarlyStopping(monitor='val_loss', patience=20, restore_best_weights=False)
_ = model.fit(X, Y, epochs=epochs, validation_split=0.1, batch_size=2, verbose=0, callbacks=[CustomCallback(), tensorboard_callback, early_stopping_callback])
model.save("models/openpose/classification/stroke_classification.keras")
